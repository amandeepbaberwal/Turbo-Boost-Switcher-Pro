#!/bin/bash
# build-install-menu.sh — compile TurboMenu, install .app + auto-start agent.
...
sleep 1
killall TurboMenu 2>/dev/null || true  # clear manually-launched strays (agent KeepAlive would resurrect)
sleep 1
...
echo "[4/4] verifying"
sleep 3
COUNT=$(pgrep -f "TurboMenu.app" | wc -l | tr -d ' ')
...
else echo "WARNING: $COUNT copies running, cleaning:"; launchctl bootout "gui/$(id -u)" "$AGENT" 2>/dev/null || true; killall TurboMenu 2>/dev/null || true; sleep 1; launchctl bootstrap "gui/$(id -u)" "$AGENT"; fi
